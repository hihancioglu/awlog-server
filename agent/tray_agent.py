"""System tray variant of the remote work agent.

This version removes the self-update and FortiClient startup features from
``agent.py`` and runs primarily from the system tray.
"""
...
LOCKFILE = os.path.join(tempfile.gettempdir(), "evden_calisma_tray.lock")
...
class MainWindow(QWidget):
    ...
    def _start_workflow(self):
        ...
        if not vpn_connected():
            ...
            self.logla("VPN yok. Bağlantı bekleniyor...")
...
if __name__ == "__main__":
    ...
    win = MainWindow()
    tray = QSystemTrayIcon(win.windowIcon())
    menu = QMenu()
    show_action = QAction("Göster", triggered=win.show)
    exit_action = QAction("Çıkış", triggered=win.quit_from_tray)
    menu.addAction(show_action)
    menu.addAction(exit_action)
    tray.setContextMenu(menu)
    tray.setToolTip("Evden Çalışma Agent")
    tray.show()
    win.hide()
    try:
        sys.exit(app.exec_())
